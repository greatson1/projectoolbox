...
interface Message {
  ...
  content: string;
  timestamp: Date;
  type?: "text" | "artefact" | "risk" | "status";
}

const QUICK_ACTIONS = [
  { label: "Status Update", icon: BarChart3, prompt: "Give me a status update on the current project including progress, risks, and blockers.", color: "#6366F1" },
  { label: "Generate Artefact", icon: FileText, prompt: "Generate a status report for the current project phase.", color: "#22D3EE" },
  { label: "Check Risks", icon: AlertTriangle, prompt: "Analyse current project risks. Flag any new risks and update existing scores.", color: "#EF4444" },
  ...
  const sendMessage = async (text?: string) => {
    ...
    try {
      ...
      if (res.ok && data.data?.agentMessage) {
        ...
        setMessages(prev => [...prev, { id: `err-${Date.now()}`, role: "agent", content: data.error || "Error. Try again.", timestamp: new Date() }]);
      }
    } catch {
      setMessages(prev => [...prev, { id: `err-${Date.now()}`, role: "agent", content: "Connection error.", timestamp: new Date() }]);
    }
        ...
        {agents.length === 0 ? (
          ...
          <button key={agent.id}
            ...
            <div className="flex-1 min-w-0">
              ...
              <p className="text-[10px] text-muted-foreground truncate">{agent.deployments?.[0]?.project?.name || "Unassigned"}</p>
            </div>
        ...
        <div ref={scrollRef} className="flex-1 overflow-y-auto p-4 space-y-4">
          {messages.length === 0 && activeAgent && (
            <div className="text-center py-8">
              ...
              <p className="text-sm text-muted-foreground mb-6">Ask questions or use quick actions below.</p>
          ...
          {activeAgent.deployments?.[0]?.project && (
            <Card>
              <CardContent className="pt-4">
                <p className="text-[10px] font-semibold uppercase tracking-wider text-muted-foreground mb-1">Project</p>
