...
Produce the full, complete document. Do not truncate or use placeholders.`,
      ...
      // Fall back to the phase NAME if the row id isn't visible yet (race during
      // initial deploy) — phase-tracker accepts both forms. NULL leaves the artefact
      // orphaned forever and silently halves the visible completion %.
      ...
      try {
        ...
        if (dep?.currentPhase) {
          ...
          autoPhaseId = phaseRow?.id ?? dep.currentPhase;
        }
